...
the code section sits encrypted in memory almost all the time; each page is decrypted only momentarily when execution actually jumps into it (via the page-fault trampoline), then re-encrypted later once the cache fills up.
